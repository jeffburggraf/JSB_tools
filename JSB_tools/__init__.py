...
def convolve_gauss(a, sigma: Union[float, int], kernel_sigma_window: int = 6, mode='same', return_kernel=False,
    ...
    """
    Simple gaussian convolution of an array.
    If array is histogram bin values, then use gaussian_broaden
...
def gaussian_broaden(bins, ys, sigma_func_or_array: Callable[[float], float]):
    """
    Performs gaussian convolution, where sigma is specified by a callable function.

    Use for, e.g., gaussian broadening spectra generated by MCNP.
    ...
    Args:
        ...
        ys: Histogram bin values. Must be length = len(bins) - 1

        sigma_func_or_array:
            Function must take "x-axis value", returns gaussian sigma in same units.
            Or,
            array/list of length equal to len(ys)

    Returns:
        np.ndarray()

    """
    if isinstance(ys[0], UFloat):
        ys = unp.nominal_values(ys)

    if not len(bins) - 1 == len(ys):
        raise ValueError("`bins` array must be one element longer than `ys` array")

    xs = 0.5 * (bins[1:] + bins[:-1])

    out = np.zeros_like(ys)

    if not hasattr(sigma_func_or_array, '__call__'):
        assert isinstance(sigma_func_or_array, (list, np.ndarray)), "Invalid type to argument `sigma_func_or_array`"

        def sigma_func(_):
            return sigma_func_or_array[i0]
    else:
        sigma_func = sigma_func_or_array

    for i0, (b0, b1) in enumerate(zip(bins[:-1], bins[1:])):
        x0 = xs[i0]
        if (weight := ys[i0]) == 0:
            continue
        ...
        _bins = bins[i_min: i_max + 1]

        # _cdfs = norm.cdf(_bins, loc=x0, scale=sigma)
        _cdfs = fast_norm_cdf(_bins, x0, sigma)

        kernel = _cdfs[1:] - _cdfs[:-1]

        corr = _cdfs[-1] - _cdfs[0]  # normalize kernel

        kernel /= corr

        out[i_min: i_max] += weight * kernel
