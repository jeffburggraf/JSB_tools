...
class LevelRecord(Record):
    def __init__(self, dataset, record, comments, xref, level_index=None):
        ...
        self.decays: List[DecayRecord] = []
        self.populating: List[DecayRecord] = []
    ...
    def add_decay(self, decay):
        ...
        # tot = [d.rel_intensity for d in self.decays]
        # for d in self.de

    def __repr__(self):
        return f"<LevelRecord; E = {self.energy} t_1/2 = {self.half_life}>"
...
class GammaRecord(DecayRecord):
    ...
    def __repr__(self):
        return f"<DecayRecord; {self.orig_level.energy} to {self.dest_level.energy}, rel. I = {self.rel_intensity}%>"
